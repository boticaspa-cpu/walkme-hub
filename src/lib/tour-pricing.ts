...
export function computeTourPrice(
  ...
  if (packageName) {
    ...
    if (adult) {
      ...
      return { adultPrice: adult.sale_price ?? 0, childPrice: child?.sale_price ?? 0, taxAdultUsd: 0, taxChildUsd: 0, source: "variant" };
    }
  }
  ...
  if (adultVariant) {
    ...
    return { adultPrice: adultVariant.sale_price ?? 0, childPrice: childVariant?.sale_price ?? 0, taxAdultUsd: 0, taxChildUsd: 0, source: "variant" };
  }
  ...
  if (packageName && tourPackages) {
    ...
    if (pkg) {
      return { adultPrice: pkg.price_adult_mxn ?? 0, childPrice: pkg.price_child_mxn ?? 0, taxAdultUsd: 0, taxChildUsd: 0, source: "tour_base" };
    }
  }
