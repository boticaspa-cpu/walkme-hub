...
import {
  DollarSign, CalendarCheck, Users, Map, FileText, ArrowRight,
  Wallet, Percent, Receipt, Tag, Package, Eye, EyeOff,
} from "lucide-react";
...
const CLIENT_MODE_KEY = "dashboard:clientMode";
...
export default function Dashboard() {
  ...
  const [clientMode, setClientMode] = useState<boolean>(() => {
    ...
    return localStorage.getItem(CLIENT_MODE_KEY) === "1";
  });
  useEffect(() => {
    localStorage.setItem(CLIENT_MODE_KEY, clientMode ? "1" : "0");
  }, [clientMode]);
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-display">
            {clientMode ? "Dashboard" : (isAdmin ? "Dashboard General" : "Mi Dashboard")}
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            {clientMode ? "Accesos rápidos" : (isAdmin ? "Resumen de operaciones de hoy" : "Tu resumen personal de hoy")}
          </p>
        </div>
        <Button
          ...
          title="Oculta cifras y datos sensibles"
        >
          {clientMode ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
          <span className="hidden sm:inline">{clientMode ? "Modo Normal" : "Modo Cliente"}</span>
        </Button>
      </div>
